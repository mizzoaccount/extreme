...
const CollectionsPage = () => {
  ...
  const brands = [...new Set(products.map(item => item.category.brand))];
  const categories = [...new Set(products.map(item => item.category.main))];
  ...
  const filteredProducts = products.filter((product) => {
    ...
    const matchesSearch = searchQuery === "" || 
      ...
      product.category.brand.toLowerCase().includes(searchLower);
    
    // Brand filter
    const matchesBrand = selectedBrands.length === 0 || 
      selectedBrands.includes(product.category.brand);
    
    // Category filter
    const matchesCategory = selectedCategories.length === 0 || 
      selectedCategories.includes(product.category.main);
    
    // Price range filter
    const matchesPrice = product.price >= priceRange[0] && product.price <= priceRange[1];
    
    // Collection theme filter
    const isNewArrival = new Date(product.createdAt).getTime() > Date.now() - 30 * 24 * 60 * 60 * 1000;
    const matchesCollection = activeCollection === "All Collections" || 
      (activeCollection === "New Arrivals" && isNewArrival);

    return matchesSearch && matchesBrand && matchesCategory && matchesPrice && matchesCollection;
  });
    ...
    <div className="bg-white min-h-screen">
      ...
      <main className="pt-24 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        ...
        >
          ...
          <p className="text-lg text-gray-600 max-w-3xl mx-auto">
            Discover premium sneakers and footwear from top brands
          </p>
        </motion.section>    
